const User = require("../model/employe")
...
exports.postUser = async (req,res)=>{
    try {
        ...
        if(userExist) return res.status(500).json({errors:true,message:"user exist"})
...
exports.login = async (req,res)=>{
    try {
        const userExist = await User.findOne({email:req.body.email})
        if(!userExist) return res.json({errros:true,message:"email or password invalird"})
